...
?>
<form name="userform" method="post" action="offices.php">
...
<?php
if($rs = $db->execute("select * from ".TBL_OFFICE." order by name"))
{
	while(!$rs->EOF)
	{
		?>
...
function showEditform($db, $id)
{
	...
	{
		$loaded = true;
		echo "<h2 class=\"edit\">Edit office</h2>";
	}
	else
	{
		$loaded = false;
		echo "<h2 class=\"edit\">Create new office</h2>";
	}
	?>
    <div id="editform">
    	<table>
    		...
    		<tr>
    			...
    			<?php if($loaded){?>
    			<input type="submit" class="icon_delete" value="delete" onclick="if(okDel('office')) {this.form.act.value='del'} else return false;" />&nbsp;
				<?php }?>
